 ...
 */
	...
	public int searchElements(Item s){
		Node searchNode= new Node(s);
		Node current =head;
		int count=0;
	...
	public static void print(){
		...
		while(current!=null){
			...
			System.out.printf( "|" + current.data + "|" , " --> " + "|" + current.data + "|"+"/n" );	
	...
	/**
	public Node partition(DoublyLinkMatrix list,Node head,Node last){
		
		
		Node smallIndex= head;
		Node index= smallIndex.next;
		Node temp= new Node();
		Item pivot= head.data;
		
		while(index!=last.next){
			
			if((index.data).compareTo(pivot) <= 0){
	             smallIndex = smallIndex.next;
	             temp.data = index.data;
	             index.data = smallIndex.data;
	             smallIndex.data = temp.data;
	                }

	         index = index.next;
	        }
		 temp.data = head.data;
         head.data = smallIndex.data;
         smallIndex.data = temp.data;
		return smallIndex;
		
	}
	private void recQuickSort(DoublyLinkMatrix list,Node head,Node last)
	{
		
		 
	        while(head != last){
	         Node pivotLocation = partition(list, head, last);
	        
	         recQuickSort(list, head, pivotLocation.previous);
	 ...
	 public void quickSort()
	{
	     ...
	     recQuickSort(this, this.head, this.tail);
	    }
